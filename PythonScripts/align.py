...
# Only works for similar waveforms and only approximates
def libFun(wave1, wave2):
	mid = len(wave1)-1
	cor = np.correlate(wave1, wave2, 'full')
	print(cor, signal.correlate(wave1, wave2, 'full'))
	return np.argmax(cor) - mid

# Gets more accurate alignment but much more time consuming
